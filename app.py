...
--    (usando as visitas com cap para consistência no período)
-- ══════════════════════════════════════════════════════════
...
UltimoProjetoPorTecnico AS (
    ...
    FROM VisitasCap
    WHERE projeto_consolidado IS NOT NULL
    ORDER BY tecnico_responsavel, dt_visita DESC
),
...
if pagina == "Ranking de Supervisores":
    ...
    with col_val_sup:
        if modo_sup == "Supervisor Atual":
            ...
            col_sup = "ultimo_supervisor"
            projetos_sup = sorted(df["ultimo_projeto"].dropna().astype(str).unique())
            ...
            col_sup = "ultimo_supervisor"
            atividades_sup = sorted(df["ultima_atividade"].dropna().astype(str).unique())
            ...
            col_sup = "ultimo_supervisor"
            projetos_sup2 = sorted(df["ultimo_projeto"].dropna().astype(str).unique())
    ...
    with col_val2_sup:
        if modo_sup == "Projeto → Atividade":
            atividades_proj_sup = sorted(df_proj_pool_sup["ultima_atividade"].dropna().astype(str).unique())
            if atividades_proj_sup:
