...
ResumoTecnicos AS (
    SELECT
        ...
        COUNT(CASE WHEN visita_valida = 'Valida' THEN 1 END)                         AS total_visitas_validas,
...
DuplicidadePorPropriedade AS (
    SELECT cpf_produtor, id_propriedade, COUNT(DISTINCT id_projeto) AS qtd_proj
    FROM public.acompanhamento_mensal_visitas
    WHERE dt_visita BETWEEN '{dt_inicio}' AND '{dt_fim}'
    GROUP BY cpf_produtor, id_propriedade
),

PropriedadesSobrepostas AS (
    SELECT
        v.tecnico_responsavel,
        COUNT(DISTINCT CASE WHEN d.qtd_proj > 1 THEN v.id_propriedade END) AS qtd_propriedades_multiplos_projetos
    FROM public.acompanhamento_mensal_visitas v
    LEFT JOIN DuplicidadePorPropriedade d
           ON v.cpf_produtor = d.cpf_produtor
          AND v.id_propriedade = d.id_propriedade
    WHERE v.dt_visita BETWEEN '{dt_inicio}' AND '{dt_fim}'
    GROUP BY v.tecnico_responsavel
),
...
Taxas AS (
    SELECT
        ...
        COALESCE(p.qtd_propriedades_multiplos_projetos, 0) AS qtd_multiplos_projetos
    FROM ResumoTecnicos r
    LEFT JOIN PropriedadesSobrepostas p ON r.tecnico_responsavel = p.tecnico_responsavel
...
Normalizado AS (
    SELECT
        ...
        COALESCE(
            ...
            / NULLIF(MAX(t.propriedades_ativas) OVER w - MIN(t.propriedades_ativas) OVER w, 0),
            1.0) AS n_prop_ativas,
        COALESCE(
            (t.total_de_visitas - MIN(t.total_de_visitas) OVER w)::NUMERIC
            / NULLIF(MAX(t.total_de_visitas) OVER w - MIN(t.total_de_visitas) OVER w, 0),
            1.0) AS n_total_visitas,
        COALESCE(
            (t.total_orientacoes_geral - MIN(t.total_orientacoes_geral) OVER w)::NUMERIC
            / NULLIF(MAX(t.total_orientacoes_geral) OVER w - MIN(t.total_orientacoes_geral) OVER w, 0),
            1.0) AS n_ori_geral,
        COALESCE(
            (t.taxa_validade - MIN(t.taxa_validade) OVER w)
            / NULLIF(MAX(t.taxa_validade) OVER w - MIN(t.taxa_validade) OVER w, 0),
            1.0) AS n_taxa_validade,
        COALESCE(
            (t.taxa_ori_concluidas - MIN(t.taxa_ori_concluidas) OVER w)
            / NULLIF(MAX(t.taxa_ori_concluidas) OVER w - MIN(t.taxa_ori_concluidas) OVER w, 0),
            1.0) AS n_taxa_ori_concluidas,
        1.0 - COALESCE(
            (t.propriedades_inativas - MIN(t.propriedades_inativas) OVER w)::NUMERIC
            / NULLIF(MAX(t.propriedades_inativas) OVER w - MIN(t.propriedades_inativas) OVER w, 0),
            0.0) AS n_prop_inativas,
        1.0 - COALESCE(
            (t.qtd_multiplos_projetos - MIN(t.qtd_multiplos_projetos) OVER w)::NUMERIC
            / NULLIF(MAX(t.qtd_multiplos_projetos) OVER w - MIN(t.qtd_multiplos_projetos) OVER w, 0),
            0.0) AS n_multi_projetos
    FROM Taxas t
    ...
    WINDOW w AS (PARTITION BY s.ultimo_supervisor)
)
...
SELECT
    ...
    ) / 7.0 AS NUMERIC), 4) AS nota_final,
    DENSE_RANK() OVER (
...
# =====================================
col_tipo, col_val = st.columns([1.5, 4.5])
with col_tipo:
    modo = st.selectbox("🔎 Filtrar por", ["Supervisor", "Projeto", "Técnico"])
with col_val:
    if modo == "Supervisor":
