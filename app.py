...
# ══════════════════════════════════════════════════════════
# TRAVA DE ACESSO — pede senha antes de mostrar qualquer dado.
# A senha correta fica em st.secrets (Settings > Secrets, no
# Streamlit Cloud), nunca no código. Sem isso configurado, o
# app não libera o acesso (evita rodar destravado por engano).
# ══════════════════════════════════════════════════════════
def _checar_senha():
    try:
    ...
    if not senha_correta:
        senha_correta = os.getenv("APP_PASSWORD")  # fallback: .env local (via load_dotenv no topo)

    if not senha_correta:
        st.error(
            "Senha de acesso não configurada.\n\n"
            "- Rodando local: adicione a linha `APP_PASSWORD=sua_senha` no arquivo `.env` do projeto.\n"
            "- Rodando no Streamlit Cloud: configure em Settings > Secrets (`APP_PASSWORD = \"sua_senha\"`)."
        )
        st.stop()
    ...
    <div style="max-width:380px;margin:12vh auto 0;text-align:center">
        ...
        <h2 style="margin:4px 0 2px">Ranking ATeG</h2>
        <p style="color:#6b7280;font-size:0.85rem;margin-bottom:18px">Acesso restrito — informe a senha para continuar.</p>
    </div>
    ...
    with tab1_sup:
        ...
        with col_chart_sup:
            ...
            indicadores_sup = {
                ...
                "n_avaliacao_subjetiva": "Avaliação",
            }
    ...
    with tab2_sup:
        ...
        for _, row in df_supv.iterrows():
            ...
            if sem_avaliacao_sup:
                ...
                _aval_equipe_bar = score_bar(row['n_avaliacao_subjetiva'], '#7c3aed')
    ...
    for _, row in df_sup.iterrows():
        ...
        with st.expander(
            ...
            painel_html = (
                ...
                + penalidade_html
                + '</div>'
                + f'<div style="font-size:0.75rem;color:#64748b;margin-bottom:4px;font-weight:600">✅ MELHOR DO GRUPO NESTES INDICADORES ({qtd_lider}/{qtd_criterios}) — nota 10:</div>'
            ...
            if pd.isna(row['avaliacao_subjetiva']):
                ...
                _tec_aval_bar = score_bar(row['n_avaliacao_subjetiva'], '#7c3aed')
