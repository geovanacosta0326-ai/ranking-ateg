...
def _texto_heatmap(row, col, colunas_absolutas):
    ...
    if col == "n_avaliacao_subjetiva":
        return "Não avaliado" if pd.isna(valor_abs) else fmt.format(safe_float(valor_abs))
...
<div class="page-header">
    ...
    <span class="badge">Nota 0 a 100%</span>
</div>
""", unsafe_allow_html=True)
...
with col_count:
    _qtd_tecnicos = len(df) if not busca_tecnico.strip() else len(df[df["tecnico_responsavel"].str.contains(busca_tecnico, case=False, na=False)])
    st.markdown(f"""
    <div class="metric-card green" style="margin-top:1px">
        <div class="label">👥 Técnicos</div>
        <div class="value">{_qtd_tecnicos}</div>
    </div>
    """, unsafe_allow_html=True)
...
if pagina == "Ranking de Supervisores":
    ...
    tab1_sup, tab_dist_sup, tab2_sup = st.tabs(["🏆 Ranking", "📊 Distribuição de Propriedades x Orientações", "🔎 Detalhes por Supervisor"])
    with tab1_sup:
        # ── Métricas gerais ──
        st.markdown(f"""
        <div class="metric-strip">
        ...
        with col_chart_sup:
            ...
            fig_sup.update_layout(
                height=max(200, len(sup_names) * 38 + 60),
                margin=dict(l=0, r=10, t=10, b=10),
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                font=dict(size=11, color="#14261a"),
                xaxis=dict(side="top", tickangle=-30, tickfont=dict(color="#14261a"), domain=[0, 0.72]),
    ...
    with tab2_sup:
        ...
        for _, row in df_supv.iterrows():
            ...
            _aval_equipe_str = "Não avaliado" if sem_avaliacao_sup else f"{safe_float(row['avaliacao_subjetiva']):.1f}"
...
tab1_tec, tab_dist_tec, tab2_tec = st.tabs(["🏆 Ranking", "📊 Distribuição de Propriedades x Orientações", "🔎 Detalhes por Técnico"])
with tab1_tec:
    # ── MÉTRICAS ─────────────────────────────────────────────────────────────────
    ...
    with col_chart:
        ...
        fig.update_layout(
            height=max(200, len(tecnicos) * 38 + 60),
            margin=dict(l=0, r=10, t=10, b=10),
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(size=11, color="#14261a"),
            xaxis=dict(side="top", tickangle=-30, tickfont=dict(color="#14261a"), domain=[0, 0.72]),
...
with tab2_tec:
    ...
    for _, row in df_sup.iterrows():
        ...
        with st.expander(
            ...
            _tec_aval_str = "Não avaliado" if pd.isna(row['avaliacao_subjetiva']) else f"{safe_float(row['avaliacao_subjetiva']):.1f}"
