...
def recalcular_scores(df_grupo):
    ...
    def minmax_neg(col):
        mn, mx = df_g[col].min(), df_g[col].max()
        # Sem variação: todos empatados (ex.: todos com 0 inativas/repetições) → todos positivos
        if mx == mn:
            return pd.Series([1.0] * len(df_g), index=df_g.index)
        return 1.0 - (df_g[col] - mn) / (mx - mn)
    ...
    with tab1_sup:
        ...
        with col_chart_sup:
            ...
            indicadores_sup = {
                ...
                "n_total_visitas":       "T. Visitas",
                "n_taxa_validade":       "T.Vistas Váliadas",
                "n_ori_geral":           "T. Orientacões",
                "n_taxa_ori_concluidas": "T.Orientacoes Concluídas",
