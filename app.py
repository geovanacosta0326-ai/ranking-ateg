...
hoje = date.today()
# Mês anterior como padrão para início e fim
if hoje.month == 1:
    _mes_pad, _ano_pad = 12, hoje.year - 1
else:
    _mes_pad, _ano_pad = hoje.month - 1, hoje.year

_anos_opcoes = [2024, 2025, 2026]
_idx_ano_pad = _anos_opcoes.index(_ano_pad) if _ano_pad in _anos_opcoes else len(_anos_opcoes)-1

col_mi, col_ai, col_mf, col_af, col_btn = st.columns([2, 1.2, 2, 1.2, 1])
with col_mi:
    mes_ini = st.selectbox("📅 Mês inicial", options=list(MESES.keys()),
                           format_func=lambda x: MESES[x], index=_mes_pad - 1, key="mes_ini")
with col_ai:
    ano_ini = st.selectbox("Ano inicial", options=_anos_opcoes, index=_idx_ano_pad, key="ano_ini")
with col_af:
    ano_fim = st.selectbox("Ano final", options=_anos_opcoes, index=_idx_ano_pad, key="ano_fim")
with col_mf:
    # Se ano final for o ano corrente, limita meses até o mês atual
    if ano_fim == hoje.year:
        _meses_fim = {k: v for k, v in MESES.items() if k <= hoje.month}
    else:
        _meses_fim = MESES
    _idx_mf = list(_meses_fim.keys()).index(_mes_pad) if _mes_pad in _meses_fim else len(_meses_fim)-1
    mes_fim = st.selectbox("📅 Mês final", options=list(_meses_fim.keys()),
                           format_func=lambda x: _meses_fim[x], index=_idx_mf, key="mes_fim")
with col_btn:
    ...
    with tab1_sup:
        ...
        with col_chart_sup:
            ...
            indicadores_sup = {
                ...
                "n_taxa_validade":       "Vistas Váliadas",
            ...
            text_abs_sup = [
                [colunas_absolutas_sup[col][1].format(safe_float(row[colunas_absolutas_sup[col][0]])) for col in indicadores_sup.keys()]
                for _, row in df_supv.iterrows()
            ]
...
with tab1_tec:
    ...
    with col_chart:
        ...
        text_abs = [
            [colunas_absolutas[col][1].format(safe_float(row[colunas_absolutas[col][0]])) for col in indicadores.keys()]
            for _, row in chart_df.iterrows()
        ]
...
with tab2_tec:
    ...
    for c in ["Score Ativas","Score Inativas","Score Visitas","Score Taxa Válidas",
              "Score Orient.","Score Taxa Concl.","Score Repet.-Proj."]:
