...
[data-testid="stAppViewContainer"] > .main > .block-container { max-width: 1200px; padding: 1.5rem 2rem 3rem; }
.page-header { background: linear-gradient(135deg,#0f172a 0%,#1e3a8a 100%); color:white; padding:20px 28px; border-radius:12px; margin-bottom:20px; display:flex; align-items:center; justify-content:space-between; }
...
with col_sel:
    sup = st.selectbox("Supervisor", supervisores, label_visibility="collapsed")
with col_btn:
...
# RADAR — 7 indicadores
# =====================================
st.markdown('<p class="section-title" style="margin-top:8px">Perfil por Indicador</p>', unsafe_allow_html=True)

cats = ["Prop. ativas","Total visitas","Orientações","Taxa válidas","Taxa concluídas","Prop. inativas","Multi-projetos"]
cols_rad = ["n_prop_ativas","n_total_visitas","n_ori_geral","n_taxa_validade","n_taxa_ori_concluidas","n_prop_inativas","n_multi_projetos"]

fig_radar = go.Figure()
for _, row in df_sup.iterrows():
    valores = [round(safe_float(row.get(c)), 4) for c in cols_rad]
    valores += [valores[0]]
    fig_radar.add_trace(go.Scatterpolar(
        r=valores, theta=cats + [cats[0]],
        fill="toself", opacity=0.35, name=row["tecnico_responsavel"],
    ))
fig_radar.update_layout(
    polar=dict(radialaxis=dict(visible=True, range=[0,1])),
    height=480, margin=dict(l=40,r=40,t=40,b=40),
    paper_bgcolor="rgba(0,0,0,0)", legend=dict(font_size=10),
    font=dict(size=11, color="#111827"),
)
